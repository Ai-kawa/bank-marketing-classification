...
# ============================================================
# Статистическая оценка модели: несколько независимых запусков
# обучения с разными случайными разбиениями train/val/test,
# чтобы убедиться, что качество модели устойчиво, а не
# результат одного удачного разбиения данных.
# ============================================================
...
print("=" * 60)
print(f"СТАТИСТИЧЕСКАЯ ОЦЕНКА МОДЕЛИ ({N_RUNS} независимых запусков)")
print("=" * 60)
...
results_df.index = [f"Запуск {i+1}" for i in range(N_RUNS)]
...
print("\n" + "=" * 60)
print("ИТОГОВАЯ СТАТИСТИКА (среднее ± стандартное отклонение)")
print("=" * 60)
for column in results_df.columns:
    mean_value = results_df[column].mean()
    std_value = results_df[column].std()
    print(f"{column:12s}: {mean_value:.4f} ± {std_value:.4f}")

results_df.to_csv("statistical_evaluation_results.csv")
print("\nРезультаты сохранены: statistical_evaluation_results.csv")


# ============================================================
# График разброса метрик по запускам
# ============================================================
...
ax.set_title(f"Статистическая устойчивость метрик ({N_RUNS} независимых запусков)")
ax.set_ylim(0, 1)
ax.grid(axis="y", alpha=0.3)

for bar, mean_value in zip(bars, means.values):
    ax.text(bar.get_x() + bar.get_width() / 2, mean_value + 0.02, f"{mean_value:.3f}",
            ha="center", va="bottom", fontsize=9)

plt.tight_layout()
plt.savefig("statistical_evaluation.png", dpi=300)
plt.close()
